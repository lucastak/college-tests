...
metadata:
  ...
  globs:
    ...
    - "service.json"
  version: "1.0"
  purpose: Build VTEX IO backend services using the node builder and @vtex/api client system
  applies_to:
    - creating REST API routes in VTEX IO
    - implementing GraphQL resolvers with backend logic
    - building event handlers and scheduled tasks
    - integrating with VTEX Commerce APIs and external services
  excludes:
    - manifest.json and builder configuration (see vtex-io-app-structure)
    - React component development (see vtex-io-react-apps)
  decision_scope:
    - which client base class to extend (JanusClient vs ExternalClient vs AppClient)
    - how to structure the Service entry point
